# 🧠 AI 面试题库

> **665 道精选面试题** · 遗忘曲线智能复习系统 · 三大间隔重复算法
>
> 对标 **阿里 P7 / 字节 2-2 / 腾讯 T9**

🌐 **在线访问**：[https://sunarthur86.github.io/ai-interview/](https://sunarthur86.github.io/ai-interview/)

## ✨ 核心功能

### 📚 题库系统
- **665 道题目**：覆盖 AI 基础、Agent 架构、RAG 工程、LLM 面试 100 问、工程化实践等
- **15 个数据文件**：数据展示分离架构，JSON 可独立编辑
- **搜索 / 筛选**：按分类、难度（L1-L5）、子分类、收藏、关键词搜索
- **详情模态**：Markdown 渲染、代码高亮、延伸追问、配图查看

### 📖 刷题模式（Flashcard）
- **三种模式**：顺序 / 随机 / 错题本
- **自评系统**：✅ 会了 / 🤔 模糊 / ❌ 不会
- **学习仪表盘**：今日刷题、连续天数、掌握率、错题数
- **每日目标**：可调节目标进度条
- **键盘快捷键**：←→/Space/123/Esc

### 🔁 遗忘曲线复习系统（v3.0 新增）
基于认知科学原理的智能间隔重复系统，自动安排最佳复习时间。

#### 三大算法可选

| 算法 | 原理 | 适用场景 |
|------|------|----------|
| **SM-2 智能间隔** | 根据记忆表现动态调整 ease factor | 最科学，推荐日常使用 |
| **Leitner 卡盒** | 5 个盒子递进，答对升级答错降级 | 直观有趣，适合初学者 |
| **艾宾浩斯曲线** | 经典 6 点：1d/2d/4d/7d/15d/30d | 经典理论，适合考前突击 |

#### 复习功能
- **4 级评分**：😵 完全忘了 / 🤔 很模糊 / ✅ 记住了 / 🌟 很轻松
- **间隔预览**：评分前显示每种评分对应的下次复习时间
- **复习仪表盘**：到期提醒、掌握进度条、算法显示
- **7 天预测**：柱状图展示未来一周复习量
- **复习历史**：每道题的最近 5 次复习记录
- **卡片标记**：主页面卡片显示「✓ 已掌握」/「🔁 待复习」标签
- **到期徽章**：复习按钮显示红色到期数量提醒

#### 配置选项
- ✅ 算法选择（3 种可切换）
- ✅ 每日复习上限（5-500 可调）
- ✅ 自动加入新题目开关
- ✅ 复习提醒通知开关
- ✅ 清空复习数据
- ✅ 数据导出（含复习进度）

### 🎨 UI/UX
- **Apple 风格设计**：衬线标题、磨砂导航栏、圆角卡片
- **深色模式**：一键切换，localStorage 持久化
- **PWA**：可安装到手机桌面，离线可用
- **响应式**：桌面/平板/手机全适配
- **无障碍**：ARIA 标签、focus-visible、reduced-motion 支持

## 🏗️ 架构

```
ai-interview/
├── data/                   # 15 个 JSON 题库文件（665 题）
├── js/
│   ├── app.js              # 主逻辑：渲染/搜索/筛选/模态/收藏
│   ├── study.js            # 刷题模式：Flashcard/自评/仪表盘
│   └── forgetting.js       # 🔁 遗忘曲线引擎（SM-2/Leitner/Ebbinghaus）
├── css/
│   ├── style.css           # 主样式（Apple 风格 + 深色模式）
│   ├── study.css           # 刷题模式样式
│   └── forgetting.css      # 复习系统样式
├── index.html              # 展示层
├── manifest.json           # PWA manifest
├── sw.js                   # Service Worker (v3)
└── .github/workflows/      # GitHub Pages 自动部署
```

## ⌨️ 快捷键

| 键 | 功能 |
|----|------|
| `⌘K` / `Ctrl+K` | 聚焦搜索 |
| `R` | 开始遗忘曲线复习 |
| `D` | 切换深色模式 |
| `← →` | 刷题/复习导航 |
| `Space` | 展开/收起答案 |
| `1 2 3` | 刷题评分（会/模糊/不会）|
| `1 2 3 4` | 复习评分（忘了/模糊/记住/轻松）|
| `Esc` | 退出 |

## 📊 数据统计

- **15 个分类**，665 道题目
- **5 个难度等级**（L1 入门 ~ L5 专家）
- **59 项自动化测试**（Playwright headless）
- **0 JavaScript 错误**

## 🛠️ 技术栈

- **纯前端**：vanilla ES6+，零框架依赖
- **数据展示分离**：JSON → JS 渲染 → HTML 展示
- **PWA**：Service Worker + Web App Manifest
- **Playwright**：headless 自动化测试

## 📄 License

MIT
