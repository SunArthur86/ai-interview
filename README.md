# 🧠 AI 面试题库

> **862 道精选面试题** · 含50道AI场景设计题 · 费曼学习法+第一性原理 · 遗忘曲线智能复习系统 · 三大间隔重复算法 · 22张SVG概念图
>
> 对标 **阿里 P7 / 字节 2-2 / 腾讯 T9 / 小红书**

🌐 **在线访问**：[https://sunarthur86.github.io/interview-ai/](https://sunarthur86.github.io/interview-ai/)

## ✨ 核心功能

### 📚 题库系统
- **862 道题目**：覆盖 LLM核心、AI Agent、AI Harness、AI场景设计、工程化实战、AI基础七大方向
- **35 个数据文件**：数据展示分离架构，JSON 可独立编辑
- **来源**：小林coding、JavaGuide、卡码笔记、牛客、小红书/B站/掘金面经等
- **费曼学习法**：每题含 🎯本质→🧒类比→📊要点，用大白话讲懂复杂概念
- **第一性原理**：每题含问题分解→公理推导→从零重建，训练深度思维
- **配图**：159张精选技术配图（公式图、架构图、流程图）+ **22张SVG概念图**（Self-Attention、KV Cache、LoRA、MoE、Transformer、RAG、ReAct、RLHF、RoPE、Agent架构、PD分离、BPE、投机采样、Beam Search、归一化、Embedding训练、梯度下降、RAG流水线等）
- **搜索高亮**：关键词匹配自动高亮，标签可点击快捷过滤
- **筛选**：按分类、难度（L1-L5）、子分类、**标签云多选**、收藏、关键词搜索
- **详情模态**：Markdown 渲染、代码高亮、费曼/第一性原理卡片、延伸追问、上下题导航（←→）、答案复制、**分享（含深度链接 #q=id）**

### 🎮 快捷操作
- **🎲 随机一题**：L 键或按钮触发，随机抽查
- **📤 进度导出**：一键复制学习进度报告（含笔记）
- **❌ 错题本导出**：导出「不会/模糊」的题目，按分类分组、附答案
- **⌨️ 快捷键面板**：? 键显示所有快捷键
- **难度分布条**：可视化当前筛选结果的 L1-L5 分布

### 📝 学习辅助
- **🚨 题目纠错**：每题详情页「报错」按钮，一键打开预填的 GitHub Issue 反馈题目错误
- **📝 个人笔记**：每题可记录个人理解/补充，自动保存到本地，卡片显示📝标记，支持导出
- **🕐 搜索历史**：搜索框聚焦显示最近搜索记录，快速复用
- **🏷️ 标签云筛选**：Top 20 高频标签多选筛选，精准定位薄弱知识点
- **🔗 深度链接**：分享题目生成直达链接 `#q=题号`，打开自动定位

### 📖 刷题模式（Flashcard）
- **三种模式**：顺序 / 随机 / 错题本
- **自评系统**：✅ 会了 / 🤔 模糊 / ❌ 不会
- **学习仪表盘**：今日刷题、连续天数、掌握率、错题数
- **每日目标**：可调节目标进度条
- **键盘快捷键**：←→/Space/123/Esc

### 🔁 遗忘曲线复习系统（v3.0 新增）
基于认知科学原理的智能间隔重复系统，自动安排最佳复习时间。

#### 三大算法可选

| 算法 | 原理 | 适用场景 |
|------|------|----------|
| **SM-2 智能间隔** | 根据记忆表现动态调整 ease factor | 最科学，推荐日常使用 |
| **Leitner 卡盒** | 5 个盒子递进，答对升级答错降级 | 直观有趣，适合初学者 |
| **艾宾浩斯曲线** | 经典 6 点：1d/2d/4d/7d/15d/30d | 经典理论，适合考前突击 |

#### 复习功能
- **4 级评分**：😵 完全忘了 / 🤔 很模糊 / ✅ 记住了 / 🌟 很轻松
- **间隔预览**：评分前显示每种评分对应的下次复习时间
- **复习仪表盘**：到期提醒、掌握进度条、算法显示
- **7 天预测**：柱状图展示未来一周复习量
- **复习历史**：每道题的最近 5 次复习记录
- **卡片标记**：主页面卡片显示「✓ 已掌握」/「🔁 待复习」标签
- **到期徽章**：复习按钮显示红色到期数量提醒

#### 配置选项
- ✅ 算法选择（3 种可切换）
- ✅ 每日复习上限（5-500 可调）
- ✅ 自动加入新题目开关
- ✅ 复习提醒通知开关
- ✅ 清空复习数据
- ✅ 数据导出（含复习进度）

### 🎨 UI/UX
- **Apple 风格设计**：衬线标题、磨砂导航栏、圆角卡片
- **深色模式**：一键切换，localStorage 持久化
- **PWA**：可安装到手机桌面，全量离线缓存（35个数据文件），stale-while-revalidate 策略
- **响应式**：桌面/平板/手机全适配
- **移动端优化**：安全区（刘海屏/Home Indicator）适配、44px 触控目标、模态框滑动关闭手势、点击反馈动画
- **SEO/分享**：Open Graph + Twitter Card meta、深度链接 `#q=id`、动态 meta 更新、富文本分享卡片
- **无障碍**：ARIA 标签、focus-visible、reduced-motion 支持

## 🏗️ 架构

```
interview-ai/
├── config.js                # 项目配置（分类映射、子分类组、品牌信息）
├── data/                    # 35 个 JSON 题库文件（862 题）
├── images/                  # 159 张技术配图（公式图/架构图/SVG概念图）
├── js/
│   ├── app.js               # 主逻辑：渲染/搜索/筛选/模态/收藏/费曼/第一性原理
│   ├── study.js             # 刷题模式：Flashcard/自评/仪表盘
│   └── forgetting.js        # 🔁 遗忘曲线引擎（SM-2/Leitner/Ebbinghaus）
├── css/
│   ├── style.css            # 主样式（Apple 风格 + 深色模式）
│   ├── study.css            # 刷题模式样式
│   └── forgetting.css       # 复习系统样式
├── scripts/
│   ├── gen_feynman_fp.py    # 费曼+第一性原理批量生成器
│   └── sync.sh              # interview-framework 同步脚本
├── docs/                    # 项目文档
├── index.html               # 展示层
├── manifest.json            # PWA manifest
├── sw.js                    # Service Worker (v14)
└── .github/workflows/       # GitHub Pages 自动部署
```

## ⌨️ 快捷键

| 键 | 功能 |
|----|------|
| `⌘K` / `Ctrl+K` | 聚焦搜索 |
| `R` | 开始遗忘曲线复习 |
| `D` | 切换深色模式 |
| `← →` | 刷题/复习导航 |
| `Space` | 展开/收起答案 |
| `1 2 3` | 刷题评分（会/模糊/不会）|
| `1 2 3 4` | 复习评分（忘了/模糊/记住/轻松）|
| `L` | 随机一题 |
| `?` | 快捷键面板 |
| `Esc` | 退出 |

## 📊 数据统计

- **7 大分类**：LLM核心 / AI Agent / AI Harness / AI场景设计 / FDE / 工程化实战 / AI基础
- **862 道题目**，35 个数据文件
- **5 个难度等级**（L1 入门 ~ L5 专家）
- **100% 费曼学习法 + 第一性原理覆盖**
- **159 张技术配图**
- **0 JavaScript 错误**

## 🛠️ 技术栈

- **纯前端**：vanilla ES6+，零框架依赖
- **数据展示分离**：JSON → config.js 映射 → JS 渲染 → HTML 展示
- **PWA**：Service Worker + Web App Manifest（全量离线缓存）
- **Open Graph**：社交媒体分享优化

## 📄 License

MIT
