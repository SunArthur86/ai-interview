...
#### ## 常见考点
1.  **为何是4倍**：为什么 MLP 中间层维度通常设定为隐藏层的 4 倍（这源于 Transformer 论文的设定，与门控机制的表达能力和计算效率平衡有关）？
2.  **参数占比**：在一个标准的 Transformer Block 中，MLP 层通常占据了模型总参数量的约 2/3，这说明了什么？
3.  **MoE 扩展**：在混合专家模型中，如何将这个密集的 MLP 层转换为稀疏的 MoE 层以增加参数量而不显著增加计算量？
4.  **激活函数**：GELU 相比 ReLU 在 Transformer 中的优势是什么？（平滑性、非零饱和区）
