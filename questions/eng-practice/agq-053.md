...
2. **副作用操作的重试**：如果工具是“发送邮件”或“扣款”，失败能重试吗？（答案：不能盲目重试，需要幂等性检查或人工介入）
3. **错误信息的可读性**：给 LLM 的错误提示需要特别处理吗？（答案：需要，要将底层报错翻译成 LLM 易懂的自然语言或结构化错误代码，方便推理）
4. **并发工具调用**：多个工具都失败了，如何处理？（答案：分别处理，若存在依赖关系（如B依赖A的结果），A失败则B不执行）
