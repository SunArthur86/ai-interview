...
3. **截断策略**：Rerank 模型通常有 max_length 限制（如 512），输入的 chunk 超长怎么办？（答案：采用 Head-Tail 截断，保留前 128 和后 384 tokens，或仅取开头）。
4. **多轮对话**：在多轮对话中，Rerank 的 Query 如何处理？（答案：必须将历史对话摘要或上一轮问题拼接到当前 Query 后再进行 Rerank）。
