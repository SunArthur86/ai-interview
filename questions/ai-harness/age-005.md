...
## 常见考点
1.  **上下文传递**：在多轮对话中，Trace ID 如何保持一致且不被中间的异步任务打断？（通常在 Metadata 中透传 Trace Context）。
2.  **成本与采样**：Agent 调用链长且 Token 成本高，全量记录 Trace 可能会导致存储爆炸，如何制定采样策略？（建议基于错误率或低频用户进行高比例采样，正常流程可只记录摘要）。
3.  **可视化难点**：如何展示 LLM 这种“文本输入输出”的非结构化 Span？（推荐将 Prompt 的前 100 字和 Summary 挂在 Span 上，完整内容存入日志并通过 Log ID 关联）。
