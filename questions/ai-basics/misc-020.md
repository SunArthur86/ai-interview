...
- **2. Continuous Batching (连续批处理 / Iteration-level Scheduling)**
  ...
  - **解决方案**: 在 Decoder 的每个生成步（Iteration）结束后，动态检测已完成或暂停的请求，移除它们并插入新请求。
...
- **## 常见考点**
  ...
  3. **迭代级调度**: 解释 Continuous Batching 与 Pre-fill 和 Decode 阶段的关系。通常 Pre-fill 会占用大量计算资源，vLLM 如何调度 Pre-fill 和 Decode 任务混合执行。
