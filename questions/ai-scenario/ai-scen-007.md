...
【RAG 评测数据流架构图】
┌─────────────┐      ┌─────────────────┐      ┌──────────────────────┐
│ Golden Set  │─────>│  RAG Pipeline   │─────>│  Evaluator Component  │
│(Query, Doc)│      │(Retriever+LLM)  │      │(RAGAS/LangSmith)      │
└─────────────┘      └─────────────────┘      └──────────────────────┘
                           │                            │
                           ▼                            ▼
                   ┌───────────────┐          ┌───────────────────┐
                   │ Retrieval Res │          │  Metrics Report   │
                   │(Context List) │          │(Recall/Faithfulness)│
                   └───────────────┘          └───────────────────┘
...
3. **如何构建高质量的评测集**：除了人工标注，有没有自动化生成数据的方法？（答：利用LLM根据文档生成不同难度的问题，再用GPT-4进行清洗和验证，称为Synthetic Data Generation）
4. **Context Recall与Context Precision的区别**：哪个指标反映了检索器的漏检？哪个反映了误检？
