...
**30 秒电梯演讲：** RAG 召回率低要系统性排查五层链路：文档处理（切分）→ 检索（Embedding/BM25）→ 排序（Rerank）→ 上下文 → 生成。检索层优化是换更强的 Embedding 模型（BGE-M3、text-embedding-3-large）加 BM25 混合检索解决关键词匹配；排序层引入 Cross-Encoder Rerank 通常是最有效的单点提升；上下文层检查 Top-K 是否含答案，含但答错则是 Prompt 或推理问题。

**展开框架：**
1. **文档处理层** — Chunk 太大减 Size 或语义切分，太小加 Overlap 合并；检查 PDF/表格解析质量。
2. **检索层（关键）** — 换更强 Embedding 模型；加 BM25 稀疏检索做 Hybrid Search 解决关键词匹配；查询太短用 HyDE 或 Query Rewrite。
3. **排序与上下文层** — Rerank 模型（Cohere Rerank、bge-reranker）通常最有效，扩大 top-k 先检 100 再 rerank 取 10；正确证据被排后检查组装顺序，太多无关内容限制 Chunk 数。

**收尾：** 我做电商客服 RAG 时——"退货""退款"等词被向量语义模糊化导致查不到政策；检索层加 BM25 混合检索 + Rerank 重排，Top-K 从 5 增到 10 再取 5，查全率从 65% 升到 92%。您想深入聊如何量化评估 RAG 的召回率，还是 Hybrid Search 的权重如何调？
