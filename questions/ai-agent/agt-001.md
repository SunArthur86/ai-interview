...
**30 秒电梯演讲：** AI Agent 是以大模型为认知核心，结合规划、记忆与工具调用，能在多步交互中根据环境反馈持续决策并完成任务的系统。核心循环是 Thought（推理）→ Action（行动）→ Observation（观察）→ Final Answer。与普通 LLM 单次文本生成的区别是：Agent 是与环境闭环的动态决策，核心组成是 Planning（拆解）、Memory（短期/长期）、Tools（手脚）。风险控制必须设最大步数熔断防止死循环。

**展开框架：**
1. **核心组成三大件** — Planning 把模糊目标拆成可执行步骤动态调整；Memory 短期上下文+长期知识避免说完就忘；Tools 是手脚如搜索、数据库、代码执行、API 调用。
2. **核心循环与自主决策** — ReAct/Tool-use 范式抽象为 Thought→Action→Observation→Final Answer；自主体现在动作空间选下一步、信息不全时澄清或假设、失败时重试换策略或请求人工。
3. **边界与风险控制** — 死循环必须设最大步数或 Token 预算熔断；破坏性操作需人机确认二次鉴权；上下文溢出用动态摘要滑动窗口；并发场景引入乐观锁。

**收尾：** 我做电商 Agent 时——曾把"取消订单"意图误映射到退款接口导致状态机死锁，后在 System Prompt 加"动作前先检查订单状态"强制规则并引入鉴权中间件解决。您想深入聊 Agent 调用工具返回 502 错误在 ReAct 哪层处理，还是长 Observation 的 Token 优化策略？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：什么是 AI Agent | "能手脚并用、自己查资料、还会根据结果改方案的数字员工。" | 类比开场 |
| 0:20 | 三大组成图 | "Planning 拆解目标，Memory 短期长期记忆，Tools 手脚调用。" | 核心组成 |
| 0:55 | ReAct 循环图 | "Thought 推理 Action 行动 Observation 观察 Final Answer 循环。" | 核心循环 |
| 1:30 | vs LLM 对比表 | "LLM 单次生成，Agent 与环境闭环动态决策，自主选下一步。" | 区别 LLM |
| 2:10 | ReAct 代码截图 | "代码：for step range 5 + parse_action + Observation 写回上下文。" | 代码演示 |
| 2:45 | 电商取消订单案例 | "实战：取消订单误映射退款接口，加检查规则+鉴权中间件解决。" | 实战案例 |
| 3:00 | 总结口诀卡 | "记住：LLM+规划+记忆+工具，ReAct 循环，必设熔断。下期讲 vs Chain。" | 收尾 |
